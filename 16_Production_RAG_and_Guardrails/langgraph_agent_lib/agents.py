...
def helpfulness_node(state: AgentState) -> Dict[str, Any]:
  ...
  {final_response}"""
    ...
    helpfulness_check_model = get_openai_model(model_name="gpt-4.1-mini", temperature=0)
    helpfulness_chain = (
...
def create_helpfulness_langgraph_agent(
    model_name: str = "gpt-4",
    temperature: float = 0.1,
    tools: Optional[List] = None,
    rag_chain: Optional[ProductionRAGChain] = None
):
    """Create a LangGraph agent that checks helpfulness.
